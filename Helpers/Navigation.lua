...
function Navigation:SearchNext()
    ...
    if DMW.Settings.profile.Gatherers.AutoLoot and DMW.Player:GetFreeBagSlots() > 0 then
...
function Navigation:Grinding()
    ...
    if not DMW.Player.Looting and DMW.Player.Target and ((DMW.Player.Target.Distance > Settings.MaxDistance and not DMW.Player.Target.Dead) or (DMW.Settings.profile.Gatherers.AutoLoot and DMW.Player.Target.Dead and UnitCanBeLooted(DMW.Player.Target.Pointer) and DMW.Player:GetFreeBagSlots() > 0)) and (DMW.Player.Target.PosX ~= EndX or DMW.Player.Target.PosY ~= EndY or DMW.Player.Target.PosZ ~= EndZ) then
